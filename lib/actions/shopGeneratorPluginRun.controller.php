...
{
    private $lipsum_name = array('Lorem', 'ipsum', 'dolor', 'sit', 'amet', 'consectetur', 'adipiscing', 'elit');
    private $lipsum_summary = array(
        'Lorem ipsum dolor sit amet, consectetur adipiscing elit.',
        'Fusce luctus posuere augue.',
        'Maecenas mattis venenatis metus sit amet fermentum.',
        'Morbi in feugiat quam.',
        'Maecenas commodo eget elit aliquam posuere.',
        'Integer vitae eros id velit cursus euismod ac eget odio.',
        'Sed volutpat tellus id tortor molestie, a varius arcu ornare.',
        'Aliquam quis risus sit amet libero mollis efficitur.',
        'Aenean condimentum lectus vel suscipit sollicitudin.',
        'Sed venenatis quis arcu nec gravida.',
    );
    private $lipsum_description = array(
...
//        $this->getResponse()->sendHeaders();
    ...
    /**
     * Generate random text, used 'lipsum_*' properties as source.
     *
     * @param string $lipsum type of lipsum (name, summary, description)
     * @param bool $lipsum shuffle lipsum strings?
     * @return string
     */
    private function getRandomText($lipsum = 'name', $shuffle = false)
    {
        $lipsum = $this->{'lipsum_' . $lipsum};
        if ($shuffle) {
            shuffle($lipsum);
        }
    ...
    protected function step()
    {
        ...
        $data = array(
            ...
            'summary' => $this->getRandomText('summary'),
        ...
        if ($this->data['images_count'] < 1) {
            ...
            $data['name'] = $this->data['prefix'] . ' ' . $this->getRandomText('name') . ' (' . $product->getId() . ')';
            $data['price'] = $price;
            ...
            $data['url'] = shopHelper::genUniqueUrl(shopHelper::transliterate($data['name']), new shopProductModel());
            ...
            if(isset($fselectable)){
                ...
                $product->setData('sku_count', $product->sku_count - 1);
